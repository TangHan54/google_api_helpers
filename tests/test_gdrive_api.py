...
### subfolder operation
# create a subfolder
created_subfolder_id = drive.create_folder(
    "test_subfolder", parents=[created_folder_id]
)
...
root_file_id = drive.upload_file(
    local_file_path="test.csv", drive_file_name="test.csv", mimetype="text/csv"
)
# upload a file to a folder
folder_file_id = drive.upload_file(
    local_file_path="test.csv",
    drive_file_name="test.csv",
    parents=[created_subfolder_id],
)
# upload files to the same folder
success_ids, failed_files = drive.upload_files(
    file_paths=["test.csv", "test2.csv", "test3.doc"], parents=[created_subfolder_id]
)
# search a file
